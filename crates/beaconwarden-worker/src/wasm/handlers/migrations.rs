...
async fn verify_cloudflare_api_token_against_url(token: &str, url: &str) -> Result<CloudflareVerifyResult> {
    ...
    let parsed: CloudflareEnvelope<CloudflareVerifyResult> = serde_json::from_str(&body)
        .map_err(|e| worker::Error::RustError(format!("Cloudflare verify returned non-JSON (status={status}): {e}")))?;
        ...
        Err(user_err) => {
            if let Some(account_id) = env_string(env, "CLOUDFLARE_ACCOUNT_ID") {
                ...
                verify_cloudflare_api_token_against_url(token, &account_url)
                    .await
                    .map_err(|account_err| worker::Error::RustError(format!(
...
pub async fn handle_migrations_up(req: &Request, env: &Env) -> Result<Response> {
    ...
    // Optional hard override: allow a dedicated migration token in environments
    // where the CI Cloudflare API token is IP-restricted.
    if let Some(required) = env_string(env, "MIGRATIONS_TOKEN") {
        if token != required {
            return error_response(req, 401, "unauthorized", "Invalid migrations token");
        }
    } else {
        // Default: verify against Cloudflare API.
